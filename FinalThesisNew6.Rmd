...
write.csv(cleaned_amendments_df, file = "cleaned_amendments_df1.csv", row.names = FALSE)
...
num_csv_files <- sum(grepl("\\.csv$", csv_file2))
num_csv_files
...
for (i in seq_along(results_list)) {
  ...
  for (j in seq_along(results_list[[i]])) {
    ...
    writeLines(results_list[[i]][j], file.path("directoryAI.1.7", file_name))
  }
}
```
...
ats3 <- TextReuseCorpus(dir = "directoryAI.1.7",
...
# Refining Text Comparisons: Filtering and Rearranging to Highlight Enhanced Amendments
```{r}
...
file_to_read3 <- "directoryAI.1/AvaazFoundation_line_184.txt"
# Read the file into a text vector
file_content3 <- readLines(file_to_read3) 
print(file_content3)
```
...
get_file_content <- function(file_name) {
  file_path <- file.path("directory_AI_2.2", paste0(file_name, ".txt"))
...
# 3. Modify the "b_result_fin" column to keep only the numeric part
merged_df_cont2 <- merged_df_cont1 %>%
  mutate(b_result_fin = as.integer(gsub("cleaned_amendments_line_", "", b_result_fin)))

# 4. Perform a right join on cleaned_amendments_df5 and merged_df_cont by Count_Column and b_result_fin
...
filtered_edges <- final_df_processed %>%
  ...
  filter(total_score_diff >= 0.05) %>%  # Filter edges with score >= 0.1
